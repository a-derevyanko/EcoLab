...
CREATE UNIQUE INDEX ix_lab2data_id
  ON lab2data (id);
CREATE INDEX ix_LAB2DATA_start_date
  ON lab2data (id, START_DATE);
...
create table lab2team (
  id        BIGINT       NOT NULL REFERENCES lab2data (id) ON DELETE CASCADE,
  version      INT,
  user_id   BIGINT      NOT NULL REFERENCES users (id) ON DELETE CASCADE,
);

CREATE INDEX ix_lab2team
  ON lab2team (user_id);

alter table lab2team add constraint uk_lab2team unique (id, user_id);

COMMENT ON TABLE lab2team IS 'Бригада, выполняющая лабораторную работу №2';
COMMENT ON COLUMN lab2team.id IS 'Идентификатор лабораторной';
COMMENT ON COLUMN lab2team.VERSION IS 'Версия изменений(контроль одновременного доступа)';
COMMENT ON COLUMN lab2team.user_id IS 'Пользователь';