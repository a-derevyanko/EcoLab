...
CREATE UNIQUE INDEX ix_lab3data_id
  ON lab3data (id);
CREATE INDEX ix_lab3data_start_date
  ON lab3data (id, START_DATE);
...
create table lab3team (
  id        BIGINT       NOT NULL REFERENCES lab3data (id) ON DELETE CASCADE,
  version      INT,
  user_id   BIGINT      NOT NULL REFERENCES users (id) ON DELETE CASCADE,
);

CREATE INDEX ix_lab3team
  ON lab3team (user_id);

alter table lab3team add constraint uk_lab3team unique (id, user_id);

COMMENT ON TABLE lab3team IS 'Бригада, выполняющая лабораторную работу №3';
COMMENT ON COLUMN lab3team.id IS 'Идентификатор лабораторной';
COMMENT ON COLUMN lab3team.VERSION IS 'Версия изменений(контроль одновременного доступа)';
COMMENT ON COLUMN lab3team.user_id IS 'Пользователь';
